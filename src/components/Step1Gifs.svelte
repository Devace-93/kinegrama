...
<section class="card bg-base-200 border border-base-300 p-4">
  ...
  <div
    ...
    data-tip={t('tip.dropzone')}
    role="button"
    aria-label={t('tip.dropzone')}
    tabindex="0"
      ...
      {#each app.gifs as g, i (g.url)}
        <div class="card bg-base-100 border border-base-300 p-2 w-40 gap-1.5">
          ...
          <button class="btn btn-xs btn-outline btn-error tooltip"
                  data-tip={t('tip.removeGif', { name: g.name })}
                  aria-label={t('tip.removeGif', { name: g.name })}
                  onclick={() => removeGif(i)}>
            {t('step1.remove')}
          </button>
        </div>
  ...
  <div class="flex gap-2 mt-4 pt-3 border-t border-base-300 sticky bottom-0 z-10 bg-base-200 -mx-4 px-4 -mb-4 pb-4 rounded-b-2xl">
    ...
    <span class="tooltip" data-tip={app.gifs.length < 1 ? t('tip.stepLocked') : t('tip.stepGo', { name: t('steps.s2') })}>
